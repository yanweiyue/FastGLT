...
class DeeperGCN(torch.nn.Module):
    def __init__(self, args,edge_index,num_nodes):
        ...
        if args['spar_adj']:
            self.adj_mask1_train = nn.Parameter(torch.ones(self.edge_num, self.num_layers,device=self.device), requires_grad=True)
        else:
            self.adj_mask1_train = nn.Parameter(torch.ones(self.edge_num, self.num_layers,device=self.device), requires_grad=False)
        ...
        for layer in range(self.num_layers):
            if conv == 'gen':
                gcn = GENConv(hidden_channels, 
                              ...
                              norm=norm, mlp_layers=mlp_layers,spar_adj=args['spar_adj'])
            else:
    ...
    def forward(self,  x, edge_index):
        ...
        if self.block == 'res+':
            h = self.gcns[0](h, self.adj_mask1_train[:,0].reshape(-1,1), edge_index)
            if self.checkpoint_grad:
                for layer in range(1, self.num_layers):
                    ...
                    if layer % self.ckp_k != 0:
                        res = checkpoint(self.gcns[layer], h2, self.adj_mask1_train[:,layer].reshape(-1,1), edge_index)
                        h = res + h
                    else:
                        h = self.gcns[layer](h2, self.adj_mask1_train[:,layer].reshape(-1,1), edge_index) + h
            else:
                for layer in range(1, self.num_layers):
                    h1 = self.norms[layer - 1](h)
                    h2 = F.relu(h1)
