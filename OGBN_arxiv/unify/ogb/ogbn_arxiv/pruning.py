...
import numpy as np
import torch
from util import get_W,compute_edge_score_from_edge_index
...
class FastScheduler:
    ...
    def _FastGLT_step(self):
        ...
        for l, (w, is_linear,is_para) in enumerate(zip(self.W, self._linear_layers_mask,self._parameters_mask)):
            ...
            if is_para:
                score_grow = torch.zeros_like(score_grad,device=score_grad.device)
                for col in range(score_grow.shape[1]):
                    if col:
                        score_grow[:,col] = torch.abs(compute_edge_score_from_edge_index(self.model.edge_index,self.model.num_nodes,current_mask[:,col].squeeze(),device=self.model.device)) # TODO
                    else:
                        score_grow[:,col] = torch.abs(compute_edge_score_from_edge_index(self.model.edge_index,self.model.num_nodes,current_mask[:,col].squeeze(),device=self.model.device))+torch.ones_like(score_grow[:,col],device=score_grad.device) # TODO
                score_grow += score_grad  # cora needs to use this
            elif is_linear:
