...
def get_weighted_layers(model, i=0, layers=None,paras=None, linear_layers_mask=None, parameters_mask=None):
    ...
    if i == 0:
        items = [(None, model)]
        for para_name,p in model._parameters.items():
            paras.append(p)
...
def exp_diversity_loss(v):
    ...
    loss = torch.exp(-0.5*torch.abs(diff).mean())
    return loss
...
def compute_edge_score_from_edge_index(edge_index, num_nodes,masked,device):
    ...
    deg_inv = deg.pow(-1)
    deg_inv.masked_fill_(deg_inv == float('inf'), 1.)
    deg_inv_sqrt = deg.pow(-0.5)
    deg_inv_sqrt.masked_fill_(deg_inv_sqrt == float('inf'), 1.)
    
    # use edge_index to cal the importance of edge
...
def add_trainable_mask_noise(pruner, c=1e-5):
    for mask in pruner.backward_masks:
        tensor_add_noise(mask,c)
            
 
def subgradient_update_mask(pruner, s2):
    for l, (w, is_linear,is_para) in enumerate(zip(pruner.W, pruner._linear_layers_mask,pruner._parameters_mask)):
        if pruner.S[l] <= 0:
            continue
        w.grad.data.add_(s2 * torch.sign(w.grad.data))
